...
def unzip_files(homework_file_paths: list[str]) -> None:
    ...
    for homework_path in homework_file_paths:
        if homework_path[-4:] == ".zip":
            with ZipFile(homework_path) as zip_object:
                ...
                files_in_parent_folder = get_file_paths(["*"], temp_dir + "/..") #does temp_dir end with "/"?
                #Renaming files
                for file_name1 in files_in_zip:
                    for file_name2 in files_in_parent_folder:
                        if file_name1 == file_name2:
                            os.rename(file_name1, file_name1[:-5] + "(FROM_ZIP)" + file_name1[-5:])
                #Flattening zip
                for file_name1 in files_in_zip:
    ...
    # if the folder still has zip files, run function again
