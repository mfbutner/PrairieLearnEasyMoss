...
def check_json_keys(json_info: Json_Info):
    ...
    # TODO: collect all error messages in a error messages list then at the end, if there are errors
    # print them and exit
    ...
    for position, assignment in enumerate(json_info):
        for key, key_type in key_types.items():  # rename value
            if get_origin(key_type) is not NotRequired:  # TODO: add method is_required to simplify double negation
                ...
                elif not is_type(assignment[key], key_type):
        ...
        if ("language" in assignment) and isinstance(assignment["language"], str) and (
                assignment["language"]).lower() not in supported_languages:
            # TODO: question_name is not guarenteed to exist at this point as we haven't finished validating
            # the JSON file
            print(
                f'Error: Given language "{assignment["language"]}" in assignment "{assignment["question_name"]}" is not one of the supported languages.')
            exit(1)
